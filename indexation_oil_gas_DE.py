...
class class_alternate(object):
    ...
    def func_lag_period(self, parameters, *data):
        ...
        values = compute_mse( preprocessing.scale(self.y) ,np.c_[np.ones(self.nbr_months_per_data),  preprocessing.scale(self.MA_func_vect(lag_oil, lag_gas, period_oil, period_gas, reset_oil,  reset_gas, np.empty(0))) ], coef) 
        return values
...
if __name__ == '__main__':
    ...
    optimization = class_alternate(df_oil ,df_gas,y ,'2014-01-31 00:00:00','2016-01-31 00:00:00', 24, 36, 12 , 12, 7, np.array([0,0,0]))
    t0 = time()
    ...
    oil_test = MA_func_vect_out(df_oil, start_day , end_day , lag[0,0] , ma_period[0,0] , reset_period[0,0] , np.empty(0))
    gas_test = MA_func_vect_out(df_gas, start_day , end_day , lag[0,1],ma_period[0,1], reset_period[0,1] , np.empty(0))
    
    X_test = np.c_[oil_test,  gas_test]
    X_test_stand = np.c_[np.ones(X_test.shape[0]), preprocessing.scale(X_test).reshape(12,2)]   
