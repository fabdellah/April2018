# -*- coding: utf-8 -*-
"""
Created on Wed Apr 18 15:13:39 2018
...
#russia_Index_x = russia_Index_x.set_index('date')
...
class class_alternate(object):
    ...
    def __init__(self, df_oil, y, start_date_str,end_date_str, nbr_months_per_data, nbr_iterations, max_lag, max_ma_period, max_reset_period ,init_coef):
            ...
            self.bounds = [(0, self.max_lag), (1, self.max_ma_period),(1, self.max_reset_period)]
    ...
    def MA_func_vect(self, lag_oil,  ma_period_oil,  reset_period_oil ,vect): #vect=np.empty(0) 
        
        start_date_oil = self.start_date - relativedelta(months=int(round(lag_oil))) - relativedelta(months=int(round(ma_period_oil) ))            
        end_date =   self.end_date + relativedelta(months=int(round(lag_oil))) + relativedelta(months=int(round(ma_period_oil) ))
    ...
    def func_lag_period(self, parameters, *data):
        ...
        lag_oil,  period_oil,  reset_oil,   = parameters
        df_oil ,  coef, values = data
        values = compute_mse( self.y.astype(float) ,np.c_[np.ones(self.nbr_months_per_data),  preprocessing.scale(self.MA_func_vect(lag_oil,  period_oil,  reset_oil,  np.empty(0))) ], coef) 
        return values


    def de_optimization(self, coef):
        """Differential evolution for the lag, ma_period and reset_period for oil, power, coal and gas."""        
        args = (self.df_oil,  coef, np.empty(1))
        result =differential_evolution(self.func_lag_period,  self.bounds, args=args)
        lag_oil, ma_period_oil, reset_period_oil,  = result.x                          
         
        return lag_oil,  ma_period_oil, reset_period_oil
    ...
    def alternate(self):
        ...
        for itera in range(self.nbr_iterations):
            ...
            t00 = time()
            lag_oil,  ma_period_oil, reset_period_oil = self.de_optimization(coef)
            t11 = time()
            d1 = t11-t00
            ...
            t02 = time()
            X_df = self.MA_func_vect(lag_oil,  ma_period_oil, reset_period_oil, np.empty(0))     
            XX_stand = np.c_[np.ones(X_df.shape[0]), preprocessing.scale(X_df).reshape(self.nbr_months_per_data,1)] 
            ...
            # update coef
            coef = np.concatenate([[res_ridge[1]],res_ridge[0][1:2]])
        ...
        return coef , lag_oil , ma_period_oil, reset_period_oil ,X_df, XX_stand, X_train, X_test, y_train, y_test
...
if __name__ == '__main__':
    ...
    #Step 1: optimizing lag, ma_period, reset_period and get the coefficients     
                                   ...
                                   #df_oil,  y, start_date_str,end_date_str, nbr_months_per_data, nbr_iterations, max_lag, max_ma_period, max_reset_period ,init_coef
    optimization = class_alternate(df_oil ,y.astype(float) ,'2010-01-31 00:00:00','2015-12-31 00:00:00', 72, 4, 7 , 7, 5, np.array([0,0]))
    t0 = time()
    coef , lag_oil , ma_period_oil, reset_period_oil , X_df, XX_stand, X_train, X_test, y_train, y_test = optimization.alternate()    
    t1 = time()
    d = t1 - t0
    ...
    end_day = '2016-12-30 00:00:00'
    oil_test = MA_func_vect_out(nbr_months_per_testing_data, df_oil, start_day , end_day , lag_oil ,  ma_period_oil ,  ma_period_oil , np.empty(0))    
    X_test_stand = np.c_[np.ones(oil_test.shape[0]), ((oil_test-mean_oil)/std_oil ).reshape(nbr_months_per_testing_data,1)]   
    ...
    oil_train = MA_func_vect_out(nbr_months_per_training_data, df_oil, start_day , end_day , 0 , 9 , 3 , np.empty(0))
    mean_3 = oil_train.mean()
    std_3 = oil_train.std()
    
    X_train_stand = np.c_[np.ones(oil_train.shape[0]), preprocessing.scale(oil_train).reshape(nbr_months_per_training_data,1)]   
    ...
    nbr_months_per_testing_data = 13   
    oil_test = MA_func_vect_out(nbr_months_per_testing_data, df_oil, start_day , end_day ,  0 , 9 , 3  , np.empty(0))   
    X_test_stand = np.c_[np.ones(oil_test.shape[0]), ((oil_test-mean_3)/std_3 ).reshape(nbr_months_per_testing_data,1)]   

    OLS_stat(X_test_stand ,y_test.astype(float))

    score(X_train_stand, y.astype(float), X_test_stand, y_test.astype(float),coef ) ## (0.19895555532031828, 0.8469227633731711)
