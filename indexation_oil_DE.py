...
@author: fabdellah
"""
# marche pas : ValueError: setting an array element with a sequence.
...
class class_alternate(object):
    ...
    def __init__(self, df_oil, start_date_str,end_date_str, nbr_months_per_year, nbr_iterations, max_lag, max_ma_period, max_reset_period ,init_coef1):
            ...
            self.bounds = tuple([(1, self.max_lag), (1, self.max_ma_period), (2, self.max_reset_period)])
    ...
    def MA_func_vect(self, lag, ma_period ,reset_period ,vect): #vect=np.empty(0) # pour reset_period=1 len(ma_vect)=11 au lieu de 12 je c pas pk
        ...
        nbr_reset_periods = int(math.ceil(self.nbr_months_per_year/int(round(reset_period))))
    ...
    def de_optimization(self,coef):
        ...
        args = (coef, np.empty(1)) #  (self.df_oil, y , coef, np.empty(1)  )
    ...
    def alternate(self):
        ...
        for itera in range(self.nbr_iterations):
            ...
            y = yy['USD.20']
            t00 = time()
            lag , ma_period, reset_period = self.de_optimization(coef)
            t11 = time()
            d1 = t11-t00
