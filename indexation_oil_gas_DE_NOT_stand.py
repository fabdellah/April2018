...
class class_alternate(object):
    ...
    def func_lag_period(self, parameters, *data):
        ...
        values = compute_mse( self.y ,np.c_[np.ones(self.nbr_months_per_data),  (self.MA_func_vect(lag_oil, lag_gas, period_oil, period_gas, reset_oil,  reset_gas, np.empty(0))) ], coef) 
        return values
    ...
    def alternate(self):
        ...
        for itera in range(self.nbr_iterations):
            ...
            XX_stand = np.c_[np.ones(X_df.shape[0]), (X_df).reshape(self.nbr_months_per_data,2)] 
...
if __name__ == '__main__':
    ...
    optimization = class_alternate(df_oil ,df_gas,y.astype(float) ,'2010-01-31 00:00:00','2016-01-31 00:00:00', 72, 6, 7 , 7, 5, np.array([0,0,0]))
    t0 = time()
