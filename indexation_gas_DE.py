# -*- coding: utf-8 -*-
"""
Created on Mon May 14 12:33:35 2018
...
class class_alternate(object):
    ...
    def __init__(self, df_gas, y, start_date_str,end_date_str, nbr_months_per_data, nbr_iterations, max_lag, max_ma_period, max_reset_period ,init_coef):
    ...
    def MA_func_vect(self, lag_gas,  ma_period_gas,  reset_period_gas ,vect): #vect=np.empty(0) 
        
        start_date_gas = self.start_date - relativedelta(months=int(round(lag_gas))) - relativedelta(months=int(round(ma_period_gas) ))            
        end_date =   self.end_date + relativedelta(months=int(round(lag_gas))) + relativedelta(months=int(round(ma_period_gas) ))
    ...
    def func_lag_period(self, parameters, *data):
        ...
        lag_gas,  period_gas,  reset_gas,   = parameters
        df_gas ,  coef, values = data
        values = compute_mse( preprocessing.scale(self.y) ,np.c_[np.ones(self.nbr_months_per_data),  preprocessing.scale(self.MA_func_vect(lag_gas,  period_gas,  reset_gas,  np.empty(0))) ], coef) 
        return values


    def de_optimization(self, coef):
        """Differential evolution for the lag, ma_period and reset_period for oil, power, coal and gas."""        
        args = (self.df_gas,  coef, np.empty(1))
        result =differential_evolution(self.func_lag_period,  self.bounds, args=args)
        lag_gas, ma_period_gas, reset_period_gas,  = result.x                          
         
        return lag_gas,  ma_period_gas, reset_period_gas
    ...
    def alternate(self):
        ...
        for itera in range(self.nbr_iterations):
            ...
            t00 = time()
            lag_gas,  ma_period_gas, reset_period_gas = self.de_optimization(coef)
            t11 = time()
            d1 = t11-t00
            ...
            t02 = time()
            X_df = self.MA_func_vect(lag_gas,  ma_period_gas, reset_period_gas, np.empty(0))     
        ...
        return coef , lag_gas , ma_period_gas, reset_period_gas ,X_df, XX_stand, X_train, X_test, y_train, y_test
...
if __name__ == '__main__':
    ...
    optimization = class_alternate(df_gas ,y ,'2010-01-31 00:00:00','2016-01-31 00:00:00', 72, 3, 12 , 12, 7, np.array([0,0]))
    t0 = time()
    coef , lag_gas , ma_period_gas, reset_period_gas , X_df, XX_stand, X_train, X_test, y_train, y_test = optimization.alternate()    
    t1 = time()
    d = t1 - t0
    ...
    start_date_index = '2016-01-01 00:00:00'                                                                   # Select dates for Russia's index
    ...
    gas_test = MA_func_vect_out(nbr_months_per_testing_data, df_gas, start_day , end_day , lag_gas , ma_period_gas , reset_period_gas , np.empty(0))    
    X_test_stand = np.c_[np.ones(gas_test.shape[0]), preprocessing.scale(gas_test).reshape(nbr_months_per_testing_data,1)]   
