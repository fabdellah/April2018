...
# -*- coding: utf-8 -*-
"""
Created on Thu May 24 10:12:37 2018

@author: farahabdellah
"""
...
facility1 =  gas_storage(6, 1, 12, 101, 0.06, 0.3, 50, 250000, 0 , 3500, -7500  ,0.01, 0.01)  
...
volume_level_stored[steps+1,:] = V_end
m_end = (V_end/alpha)
...
b = 0
for t  in range(steps,0,-1):   
    m_level = ( (volume_level_stored[t+1,0])/alpha).astype(int) + 1
    print('At time ', t , ', inject/withdraw: ',decision_rule[t,b,m_level] )              
    if (decision_rule[t,b,m_level] != 0):                   
           volume_level_stored[t,b] = volume_level_stored[t+1,0] - (np.sign(decision_rule[t,b,m_level]))*((abs(decision_rule[t,b,m_level])//alpha)+1)*alpha 
    else:
...
t0 = time()
final_contract_value = facility1.price()
print ('Price: ' , final_contract_value  )   
t1 = time();
d1 = t1 - t0
...
import matplotlib.pyplot as plt
time = np.arange(0,14)
plt.plot(time,volume_level_stored[:,0])   
plt.title('Volume level stored')
plt.xlabel('Time')
plt.ylabel('Volume')
plt.show()
