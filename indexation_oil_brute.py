...
class class_alternate(object):
    ...
    def func_lag_period(self, parameters, *data):
        ...
        values = compute_mse(y ,np.c_[np.ones(12),  self.MA_func_vect(lag1, period1, reset1, np.empty(0)) ], coef) 
        return values
    ...
    def alternate(self):
        ...
        for itera in range(self.nbr_iterations):
            ...
            XX_stand = np.c_[np.ones(X_df.shape[0]), preprocessing.scale(X_df).reshape(self.nbr_months_per_year,1)] 
...
if __name__ == '__main__':
    ...
    optimization = class_alternate(df_oil, '2016-01-31 00:00:00','2017-01-31 00:00:00', 12, 5, 4, 3, 6, np.array([0,0,0]) )
    t0 = time()
